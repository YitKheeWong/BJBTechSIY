<?php
  ...
  if($_SESSION['sid'])
  {
    ...
    if(count($_POST)>0){
        mysqli_query($con,"UPDATE Aktiviti set Keadaan = '1' WHERE Nama = '".$_GET['id']."' ");
...
<body>
    ...
    <form method="POST" action="viewMoreUnapprovedAd.php?id=<?php echo $rowAct["Nama"]; ?>">        
    <p align='center'>
        <input name='back' type='button' value='Kembali' class='button' onClick="parent.location='viewUnapprovedActAd.php'">
        <input name='Approve' type='submit' value='Setuju' class='button' onclick='approveActivity()'>
    </p>

    <p align='center'>
        Untuk kemaskini, sila tekan <a href="updateActAd.php?id=<?php echo $rowAct["Nama"];?>">sini</a>.
        Untuk padamkan, sila tekan <a href="deleteActAd.php?id=<?php echo $rowAct["Nama"];?>">sini</a>.<br>
    </p>
    </form>
