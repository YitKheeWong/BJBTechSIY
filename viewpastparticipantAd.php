<?php
  ...
  if($_SESSION['sid'])
  {
    ...
    if (isset($_POST['del']))
    {
        $resultd = mysqli_query($con, "DELETE FROM participant WHERE AName= '".$_GET['id']."'");
        if ($resultd)
        {
            echo "<script> alert(\"Rekod peserta aktiviti ini telah berjaya dihapus!\") </script>";
...
<body>
    ...
    <br><br>
    <form name="deleteParticipant" method="POST" action="<?php $_PHP_SELF?>">
    <p>Jumlah peserta: <?php echo $rowCnt['Count'];?></p>
    <table>
        <tr>
            <th>Nama Peserta</th>
        ...
        if ($resultAct->num_rows >0)
        {
            while($rowAct = $resultAct->fetch_assoc())
            {
                echo "<tr><td>" .$rowAct['Fname']. " " . $rowAct['Lname']. "</td>
                      <td>" .$rowAct['status']. "</td>
                      <td><a href=\"viewpPastParticipantAd.php?id=" .$rowAct['ID']. "\">Maklumat Lanjut</a></td></tr>";
            }
        }
        ?>
    </table>
    ...
    <p align='center'>
        <input name='back' type='button' value='Kembali' class='button' onClick="parent.location='viewMorepAd.php?id=<?php echo $_GET['id']?>'">
        <input name='del' type='submit' value='Hapus Rekod' class='button'>
    </p>
        </form>
