<?php
    ...
    $resultAct = mysqli_query($con,"SELECT * FROM Aktiviti WHERE Keadaan = '1' AND Peserta = 'Orang Awam' AND tarikhPermulaan > CURDATE() ORDER BY tarikhPermulaan ASC");
...
<div class="nv">
   ...
   <?php
        ...
        $sqlAct = "SELECT * FROM Aktiviti WHERE Keadaan = '1' AND Peserta = 'Orang Awam' AND tarikhPermulaan > CURDATE() LIMIT $this_page_first_result,$results_per_page";
    ...
    <tr class="<?php if(isset($classname)) echo $classname;?>"><?php $timeStart=date('h:i A', strtotime($row["masaPermulaan"]));?>
    <td><?php echo $row["Nama"]; ?></td>
    <td><?php echo $row["tarikhPermulaan"]; ?></td>
    <td><?php echo $timeStart; ?></td>
    <td><a href="viewMore.php?id=<?php echo $row["Nama"]; ?>">Maklumat Lanjut</a></td>
    </tr>
    <?php
    $i++;
    }
    ...
    <?php
        ...
        for($page = 1;$page<=$number_of_pages;$page++){
            echo '<a href = "viewActivity.php?page='.$page.'">'.$page.'</a> ';
        }
        mysqli_close($con);
    ?>
    </div>
